...
class TableDay extends TableBase
{
    ...
    boolean addDayItem(DayItem dayItem)
    {
        ...
        String lSql = "INSERT INTO day " +
            "  (holidayId, dayId, dayName, dayPicture, sequenceNo, dayCat, infoId, " +
            "   noteId, galleryId, sygicId) " +
            "VALUES " +
            "(" +
            dayItem.holidayId + "," +
            dayItem.dayId + "," +
            MyQuotedString(dayItem.dayName) + "," +
            MyQuotedString(dayItem.dayPicture) + "," +
            dayItem.sequenceNo + ", " +
            dayItem.dayCat + ", " +
            dayItem.infoId + ", " +
            dayItem.noteId + ", " +
    ...
    boolean updateDayItem(DayItem dayItem)
    {
        ...
        {
            if (dayItem.origPictureAssigned && dayItem.dayPicture.length() > 0 &&
        ...
        String lSQL;
        lSQL = "UPDATE day " +
            "SET dayName = " + MyQuotedString(dayItem.dayName) + ", " +
            "    dayPicture = " + MyQuotedString(dayItem.dayPicture) + ", " +
            "    dayId = " + dayItem.dayId + ", " +
            "    sequenceNo = " + dayItem.sequenceNo + ", " +
            "    dayCat = " + dayItem.dayCat + ", " +
            "    infoId = " + dayItem.infoId + ", " +
            "    noteId = " + dayItem.noteId + ", " +
            "    galleryId = " + dayItem.galleryId + ", " +
            "    sygicId = " + dayItem.sygicId + " " +
            "WHERE holidayId = " + dayItem.holidayId + " " +
            "AND dayId = " + dayItem.origDayId;
    ...
    boolean getDayItem(int holidayId, int dayId, DayItem litem)
    {
        ...
        String lSQL;
        lSQL = "SELECT holidayId, dayId, dayName, dayPicture, sequenceNo, dayCat, infoId, " +
            " noteId, galleryId, sygicId " +
            "FROM Day " +
    ...
    {
        String lSql = "SELECT holidayId, dayId, dayName, dayPicture, sequenceNo, dayCat, infoId, " +
            " noteId, galleryId, sygicId " +
            "FROM day " +
            "WHERE holidayId = " + holidayId + " " +
            "ORDER BY SequenceNo ";
    ...
    private boolean getStartEndAt(DayItem dayItem)
    {
        ...
        {
            dayItem.start_at = "";
            String lSQL = "SELECT schedName " +
                "FROM schedule " +
                "WHERE holidayId = " + dayItem.holidayId + " " +
                "AND dayId = " + dayItem.dayId + " " +
                "AND schedType = " + _resources.getInteger(R.integer.schedule_type_hotel) + " " +
                "AND startHour < 12";
