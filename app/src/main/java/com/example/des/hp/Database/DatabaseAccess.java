...
public class DatabaseAccess extends SQLiteOpenHelper
{
    ...
    private boolean createHolidays()
    {
        try
        {
            if(!createHoliday(false, false, false, false))
                return (false);
            if(!createHoliday(false, false, false, true))
                return (false);
            if(!createHoliday(false, false, true, false))
                return (false);
            if(!createHoliday(false, false, true, true))
                return (false);
            if(!createHoliday(false, true, false, false))
                return (false);
            if(!createHoliday(false, true, false, true))
                return (false);
            if(!createHoliday(false, true, true, false))
                return (false);
            if(!createHoliday(false, true, true, true))
                return (false);
            if(!createHoliday(true, false, false, false))
                return (false);
            if(!createHoliday(true, false, false, true))
                return (false);
            if(!createHoliday(true, false, true, false))
                return (false);
            if(!createHoliday(true, false, true, true))
                return (false);
            if(!createHoliday(true, true, false, false))
                return (false);
            if(!createHoliday(true, true, false, true))
                return (false);
            if(!createHoliday(true, true, true, false))
                return (false);
            if(!createHoliday(true, true, true, true))
                return (false);
            return (true);
        }
        catch(Exception e)
    ...
    private boolean createDays(int lHolidayId)
    {
        try
        {
            if(!createDay(lHolidayId, false, false, false))
                return (false);
            if(!createDay(lHolidayId, false, false, true))
                return (false);
            if(!createDay(lHolidayId, false, true, false))
                return (false);
            if(!createDay(lHolidayId, false, true, true))
                return (false);
            if(!createDay(lHolidayId, true, false, false))
                return (false);
            if(!createDay(lHolidayId, true, false, true))
                return (false);
            if(!createDay(lHolidayId, true, true, false))
                return (false);
            if(!createDay(lHolidayId, true, true, true))
                return (false);
            return (true);
        }
        catch(Exception e)
    ...
    private boolean createGeneralAttraction(int lHolidayId, int lDayId, int lAttractionId, int lAttractionAreaId)
    {
        try
        {
            ...
            for(int i=0; i < 32; i++)
            {
                boolean lFirstFlag=(i & 1) == 1;
                boolean lSecondFlag=(i & 2) == 2;
                boolean lThirdFlag=(i & 4) == 4;
                boolean lFourthFlag=(i & 8) == 8;
                boolean lFifthFlag=(i & 16) == 16;
